...
parser.add_argument('--hyper_semi',type=float, default=1.0,help='hyper of semi-loss')
parser.add_argument('--th',type=float, default=0.98,help='th')
...
for element in test_batches.take(1):
    ...
    # print(attribute_batch[:,:,0],attribute_batch[:,:,-1])
    pass
...
def fearture_extractor(inputs, model):
    ...
    prediction_batch = tf.keras.layers.Dense(num_class,kernel_regularizer=regularizer_last,bias_regularizer=regularizer_last)(feature_batch)
...
Classifier.summary()
Classifier.load_weights('./Pretrained_CNN/CNN_partial_init_%d' %index[label_ratio])
# Classifier.load_weights('./checkpoints_initial_0818_3/CNN_partial_init_%d' %index[label_ratio])
# Classifier.load_weights('./checkpoints_initial_0826_20/CNN_partial_init_20_3_reg')
...
BAE = BAE_model()
BAE.summary()
BAE.load_weights('./Pretrained_AE/BAE_partial_init_%d' %index[label_ratio])
# BAE.load_weights('./checkpoints_initial_0818_6/BAE_partial_init_%d' %index[label_ratio])
# BAE.load_weights('./checkpoints_initial_0826_1/BAE_partial_init_%d' %index[label_ratio])
# BAE.load_weights('./checkpoints_initial_0826_20/BAE_partial_init_20_3_reg')
...
AE_ft_optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.001, rho=0.9, momentum=0.9, epsilon=1.0)
...
@tf.function
def train_step(image_batch,label_batch,epoch,label_batch_true):
  with tf.GradientTape() as cl_tape, tf.GradientTape() as ae_tape:
    [feature_batch,prediction_batch,output_batch] = Classifier(image_batch, training=False)
    decoded_batch=tf.cast(output_batch > 0.5, dtype=tf.float32)*2.0 - 1.0
    cnn_accuracy = tf.reduce_mean(tf.cast(tf.equal(decoded_batch, label_batch_true), dtype=tf.float32))
    ...
    ae_decoded_binary=tf.cast(out_label > 0.0, dtype=tf.float32)
    # out_label = tf.clip_by_value(out_label, clip_value_min=-1, clip_value_max=1)

    ae_decoded_batch = ae_decoded_binary*2.0 - 1.0
    ae_accuracy = tf.reduce_mean(tf.cast(tf.equal(ae_decoded_batch, label_batch_true), dtype=tf.float32))

    batch_size = tf.shape(feature_batch)[0]


    hard_label2 = tf.cast(-1.*tf.keras.losses.cosine_similarity(ae_decoded_binary[:batch_size // 2],  ae_decoded_binary[batch_size // 2:2*(batch_size // 2)], axis=1)>0.95, dtype=tf.float32)
    hard_label = tf.cast(-1.*tf.keras.losses.cosine_similarity(out_label[:batch_size // 2],  out_label[batch_size // 2:2*(batch_size // 2)], axis=1)>th, dtype=tf.float32)
    ...
    BCE_loss = tf.compat.v1.losses.sigmoid_cross_entropy(multi_class_labels=labels_binary, logits=prediction_batch,weights=mask_batch) + hyper_semi*semi_loss
  ...
  return cnn_accuracy, ae_accuracy, BCE_loss, AE_loss, output_batch,out_label_ae, semi_loss, hard_label, hard_label2

@tf.function
def train_step2(image_batch,label_batch,epoch,label_batch_true):
  with tf.GradientTape() as cl_tape, tf.GradientTape() as ae_tape:
    [feature_batch,prediction_batch,output_batch] = Classifier(image_batch, training=False)
    decoded_batch=tf.cast(output_batch > 0.5, dtype=tf.float32)*2.0 - 1.0
    cnn_accuracy = tf.reduce_mean(tf.cast(tf.equal(decoded_batch, label_batch_true), dtype=tf.float32))
  ...
  # CNN_optimizer.apply_gradients(zip(Classifier_gradients, Classifier.trainable_variables))
  BAE_gradients = ae_tape.gradient(AE_loss, BAE.trainable_variables)
  AE_ft_optimizer.apply_gradients(zip(BAE_gradients, BAE.trainable_variables))
...
def test_batch(image_batch,label_batch, epoch):
  ...
  ae_decoded_batch = tf.cast(out_label > 0.0, dtype=tf.float32)*2.0 - 1.0
...
def fit_phase1(train_ds, valid_ds, test_ds,lr,lr_ae,epochs):
  ...
  for epoch in range(epochs):
    ...
    for n, element in train_ds.enumerate():
      ...
      accuracy_train, accuracy_train_ae, BCE_loss, AE_loss, output_batch, output_batch_ae, SEMI_loss , hard_label, hard_label2= train_step(image_batch, label_batch, epoch,label_batch_true)

      # print(hard_label, hard_label2)

      count+=np.sum(hard_label)
    ...
    if label_ratio ==5:
      ...
      threshold = map_ae_valid

    if threshold >= best_mAP:

      Classifier.save_weights('./Final/CNN_%d_conv' % index[label_ratio])
      BAE.save_weights('./Final/BAE_%d_conv' % index[label_ratio])
...
def fit_phase2(train_ds, test_ds, epochs):
  ...
  Classifier.load_weights('./Final/CNN_%d_conv' %index[label_ratio])
  BAE.load_weights('./Final/BAE_%d_conv' %index[label_ratio])
  res_mAP = []
  res_mAP_test_cnn = []
  res_mAP_test_ae = []
  ...
  for epoch in range(10):
    ...
    for n, element in train_ds.enumerate():
      ...
      accuracy_train, accuracy_train_ae, BCE_loss, AE_loss, output_batch, output_batch_ae, SEMI_loss , hard_label, hard_label2= train_step2(image_batch, label_batch, epoch,label_batch_true)
